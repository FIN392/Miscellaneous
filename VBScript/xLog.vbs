...
Set objMyLog01 = LogOpen( "S:\NO_EXISTE.LOG" )
Set objMyLog02 = LogOpen( Replace( WScript.ScriptFullName, ".vbs", ".log" ) )

LogWrite objMyLog01, "[!]"  & vbTab & "Example #1 of warning message"

LogWrite objMyLog02, "[!]"  & vbTab & "Example #1 of warning message"
LogWrite objMyLog02, "[X]"  & vbTab & "Example #2 of error message"

Set objMyLog01 = LogClose( objMyLog01 )
Set objMyLog02 = LogClose( objMyLog02 )
...
'
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
Function LogOpen( strLogFile )
On Error resume next
	
	Set LogOpen = Nothing
	
	If strLogFile = "" Then
		strLogFile = Left( WScript.ScriptFullName, InStrRev( WScript.ScriptFullName, "." ) - 1 ) & _
			"_" & Year( Now ) & Right( "0" & Month( Now ), 2 ) & Right( "0" & Day( Now ), 2 ) & _
			"-" & Right( "0" & Hour( Now ), 2 ) & Right( "0" & Minute( Now ), 2 ) & Right( "0" & Second( Now ), 2 ) & _
			".log"
	Else
		strLogFile = CreateObject( "Scripting.FileSystemObject" ).GetAbsolutePathName( strLogFile )
	End If
	
	Set LogOpen = CreateObject( "Scripting.FileSystemObject" ).OpenTextFile( strLogFile, 8, True, 0 )
	If Oops( Err, "Create log file" ) Then Exit Function
...
'
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
Sub LogWrite( objLogFile, strLogLine )
On Error resume next

	objLogFile.WriteLine Now & vbTab & strLogLine
	If Oops( Err, "Write on log file" ) Then Exit Sub
...
'
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
Function LogClose( objLogFile )
On Error resume next

	objLogFile.Close
	Set objLogFile = Nothing

	Set LogClose = Nothing
...
WScript.Echo Now & vbTab & ""  & vbTab & "Example of information message"
WScript.Echo Now & vbTab & "(!)"  & vbTab & "Example of warning message"
WScript.Echo Now & vbTab & "(X)" & vbTab & "Example of error message"
...
myLogger.IncludeTimestamp = True    ' enable time stamp
...
myLogger.Overwrite = True           ' overwrite log file (default is to append)
                                    ...
                                    ' facilities.
myLogger.LogFile = ".\TEST.log"      ' enable logging to file C:\my.log
